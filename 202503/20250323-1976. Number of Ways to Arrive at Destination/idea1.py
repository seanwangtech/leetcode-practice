...
class Solution:
    def countPaths(self, n: int, roads: List[List[int]]) -> int:
        N = n
        neighbors:List[List[Tuple]] = [[] for x in range(N) ]
        for u,v, t in roads:
            neighbors[u].append((v,t))
            neighbors[v].append((u,t))
        shortestTime = float('inf')
        numberOfWays = 0
        seen = set()
        def helper(node=0, jtime=0):
            nonlocal N, shortestTime, numberOfWays, seen
            if (jtime> shortestTime):
                return
            if(node == (N-1)):
                if jtime == shortestTime:
                    numberOfWays += 1
                elif jtime < shortestTime:
                    numberOfWays = 1
                    shortestTime = jtime
                return
            seen.add(node)
            for u, t in neighbors[node]:
                if u not in seen:
                    helper(u, jtime+t)
            seen.remove(node)
        helper()
